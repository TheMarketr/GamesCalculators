...
const latestToolDate = publishedTools.reduce((latest, tool) => tool.updated > latest ? tool.updated : latest, '2026-08-27');
...
const entries = [
  ...
  ...games.map((game) => ({ path: `/${game.slug}/`, lastmod: game.tools.reduce((latest, tool) => tool.updated > latest ? tool.updated : latest, latestGuideDate), priority: '0.9', frequency: 'weekly' })),
  ...publishedTools.map((tool) => ({ path: `/${tool.game.slug}/${tool.slug}/`, lastmod: tool.updated, priority: '0.8', frequency: 'weekly' })),
