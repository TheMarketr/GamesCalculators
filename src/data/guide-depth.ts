...
const gameProfiles: Record<string, GuideDepthProfile> = {
  ...
  'pokemon-go': {
    context: 'Pokémon GO calculations combine species records, individual IVs, Pokémon level and a CP multiplier. Evolution changes the species base stats while normal powering up changes level; neither action should be described as rerolling IVs. League planning adds a CP cap, while Candy, Candy XL and Stardust determine whether a mathematically useful target is practical.',
    measurement: 'Identify the exact species or form, record level in half-level increments and enter Attack, Defense and Stamina IVs separately. For an evolution, keep level and IVs fixed while changing the species record. For PvP, evaluate every eligible level beneath the selected cap and compare stat product within that one species and league.',
    verification: 'Check the Pokémon summary and appraisal before spending resources. Alternate forms, Shadows, Megas and game updates can change which record or modifier applies. Official help documents player-facing behavior, while the CP formula and Game Master fields are community interpreted; pages should label that distinction and show the review date.',
    scenario: 'A low-Attack appraisal can look worse by total IV percentage yet reach another half level under the Great League cap. The resulting Defense and Stamina may raise stat product. That does not make it universally stronger, because moves, breakpoints and team matchups remain outside a pure IV ranking.',
  },
  osrs: {
    context: 'Old School RuneScape planning mixes fixed integer formulas with activity rates that depend on attention, gear, banking and encounter mechanics. XP thresholds and the complement rule for independent drops are deterministic, while XP per hour, kill speed and market profit should be treated as dated or player-measured inputs.',
    measurement: 'Use exact total XP rather than level alone, preserve the documented floors in combat formulas and count only eligible personal reward rolls. When projecting actions, divide the remaining XP by XP per action and round upward. When projecting time, include banking, respawns and ordinary interruptions in the measured rate.',
    verification: 'Use the OSRS Wiki formula or table cited on the page and confirm that the selected game mode, item, monster and training method match the live account. Grand Exchange snapshots can age quickly, and an encounter may use multiple or conditional rolls rather than one simple denominator.',
    scenario: 'A 1-in-512 item has only about a 63 percent chance to appear within 512 independent kills. The expected count is useful for comparison but does not create a guarantee. The same care applies to a level goal: level 92 sounds close to 99, yet it represents only about half the total XP.',
  },
  palworld: {
    context: 'Palworld breeding, Potential and work planning use different mechanics that should not be blended. Species outcome comes from special combinations or hidden breeding ranks; HP, Attack and Defense Potential affect combat stats; Work Speed depends on suitability and work-specific bonuses instead.',
    measurement: 'For breeding, preserve exact Pal identifiers and test special pairs before applying the rank average. For Potential, record species, level, displayed stats and every relevant modifier, then keep all integer values compatible with the rounded display. For workload, convert effective Work Speed into completed workload per second.',
    verification: 'Palworld updates can add Pals, special combinations, passive skills or revised formulas. Confirm the exact subspecies and current passive descriptions against the cited wiki revision. A reverse stat result may remain a range because the displayed number has already been rounded.',
    scenario: 'Two parents can produce a special child even when their averaged ranks point elsewhere, so the exception table must run first. After selecting the right child species, the player still needs separate checks for desired passives and Potential. One correct breeding output does not guarantee the final build.',
  },
  'gta-6': {
