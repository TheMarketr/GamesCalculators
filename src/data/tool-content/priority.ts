...
export const priorityToolContent = defineToolContent({
  ...
  'blox-fruits/trade-calculator': {
    reviewed: '2026-08-28',
    methodSummary: 'Totals both offers from editable community references, compares the gap, and keeps demand visible beside value.',
    intro: [
      'The Blox Fruits Trade Calculator compares every fruit on your side with every fruit offered by the other player. It totals the selected community reference values, measures the percentage difference, summarizes demand, and labels the numerical gap so a multi-fruit offer is easier to inspect.',
      'Blox Fruits has no official cash price for player trades. The values in this tool are community planning references, not developer-set exchange rates, and the calculator keeps demand separate because two equal totals can have very different trading liquidity.',
    ],
    ...
    steps: ['Add each fruit on your side and set the correct quantity.', 'Add every fruit offered by the other trader, including small adds.', 'Review both total reference value and average demand.', 'Use the percentage gap as a negotiation aid, then consider storage, trade limits, demand, and your own goals.'],
    howItWorks: {
      title: 'How This Blox Fruits Trade Calculator Works',
      body: ['For each offer, the calculator multiplies a fruit’s editable local reference value by its quantity and any selected multiplier, then sums the rows. Difference equals their total minus your total. Percentage difference uses your total as the comparison base, so a positive percentage means their entered offer is numerically higher relative to yours.', 'The displayed verdict uses a visible fairness band around the totals. Average demand is calculated separately from value so the result does not hide whether the bundle contains fruits that are difficult or easy to move in the community market.'],
      points: ['A positive difference favors your side under the entered references.', 'A negative difference means your entered offer total is higher.', 'Demand can change the practical appeal even when totals fall inside the numerical fair band.'],
    },
    workedExample: {
      title: 'Worked Example: comparing a main fruit and adds',
      body: ['Suppose your side contains one fruit with an editable reference of 8,000,000. The other side contains one fruit at 7,400,000 plus an add at 900,000. Their total is 8,300,000, the gap is +300,000, and the difference is 3.75% of your 8,000,000 total. That falls inside the calculator’s ±7% numerical fair band.', 'The decision is not finished: if the 7,400,000 fruit has weak demand or the add is difficult to retrade, the higher arithmetic total may still be less useful to you.'],
    },
    howToUseResult: {
    ...
    limitations: {
      title: 'Community Value and Demand Limits',
      body: ['The publisher does not maintain these player-market values. Community references can move after reworks, events, supply changes, or trading trends, and different lists may disagree. Permanent products, physical fruits, stored fruits, and in-game trade restrictions can also carry different context that a single normalized reference cannot capture.'],
      points: ['Values are community reference data, not official prices.', 'Demand ratings summarize trading interest; they are not guaranteed sale speed.', 'The tool does not verify ownership, legitimacy, or the other player’s final confirmation.'],
    },
    ...
    faqs: [
      ...
      { question: 'Are the fruit trade values official?', answer: 'No. They are editable community planning references. Blox Fruits does not publish an official player-to-player exchange-rate table.' },
  ...
  'blox-fruits/value-calculator': {
    reviewed: '2026-08-28',
    methodSummary: 'Sums fruit quantities from an editable local reference and reports total value with demand context.',
    intro: ['The Blox Fruits Value Calculator builds a one-sided fruit list and totals its editable community reference value. It is designed for checking an inventory segment, preparing one side of a trade, or seeing which fruit contributes most to a bundle without mixing the result with the other trader’s offer.', 'Alongside the total, the tool keeps demand visible. Value estimates size; demand describes trading interest. Keeping them separate prevents a pile of difficult-to-move fruits from looking automatically equivalent to one popular fruit.'],
    ...
    howItWorks: { title: 'How This Blox Fruits Value Calculator Works', body: ['Each row contributes fruit reference value × quantity. The calculator adds those contributions for a bundle total and combines the selected demand scores into an average demand indicator. It does not turn demand into hidden value; both measurements remain visible.', 'The value list is a community reference snapshot stored with the site. It is useful for consistent comparison, but it is not a claim that every server or trader will accept the same number.'], points: ['Quantity multiplies value but does not make a low-demand fruit more liquid.', 'The total describes only the fruits selected on this page.', 'Use the trade calculator for an explicit your-offer versus their-offer verdict.'] },
    workedExample: { title: 'Worked Example: totaling a three-fruit bundle', body: ['If a bundle contains two fruits referenced at 1,200,000 each and one fruit referenced at 3,500,000, its total is (2 × 1,200,000) + 3,500,000 = 5,900,000. The 3,500,000 fruit contributes about 59% of the bundle, so its demand will heavily influence how the bundle feels in a real trade.'], points: ['Removing one 1,200,000 fruit lowers the total to 4,700,000.', 'A value-equivalent replacement can still alter average demand.', 'The example explains the arithmetic and does not assert current values for named fruits.'] },
    ...
    limitations: { title: 'Blox Fruits Value Data Limitations', body: ['Community fruit values change and can differ between lists. Reworks, availability, stock cycles, player preference, and permanent-versus-physical context can affect offers. The calculator does not claim official prices and does not verify whether a reference has changed since the editorial review date.'], points: ['The local dataset needs ongoing expansion and review as fruits change.', 'Player overpay for desired upgrades is not predicted.', 'A total cannot detect scams, trade-window swaps, or account risk.'] },
    ...
    faqs: [
      ...
      { question: 'Can I treat the total as an official Blox Fruits price?', answer: 'No. Player-market references are community-maintained and can move. Confirm important trades against current market activity and the final in-game offer.' },
    ],
  },
  ...
  'grow-a-garden/trade-calculator': {
    reviewed: '2026-08-28',
    methodSummary: 'Adds item rows on both sides, compares the value gap, and reports demand without treating it as currency.',
    intro: ['The Grow a Garden Trade Calculator organizes two multi-item offers and compares their editable value references. It is meant for crop, pet, and item trades where the number of entries makes mental addition unreliable.', 'The calculator shows both the raw difference and the percentage gap, then keeps demand visible as a second decision signal. A trade can be numerically close while one side contains event items, mutated crops, or low-demand adds that are harder to exchange again.'],
    useCases: ['Check an offer containing several crops or garden items.', 'Measure how much one add changes a proposed W/F/L result.', 'Compare value and demand before giving up a scarce event item.'],
    steps: ['Add every item and quantity on your offer side.', 'Build the other player’s offer without omitting small adds.', 'Confirm that mutation, growth, or variant value is represented by the selected row or multiplier.', 'Read value gap, percentage, and demand together before accepting in game.'],
    howItWorks: { title: 'How This Grow a Garden Trade Calculator Works', body: ['Each side totals item reference value × quantity × the selected row multiplier. The calculator subtracts your total from their total and divides that gap by your total for the percentage comparison. Its Fair band covers differences within ±7% of your entered offer.', 'Demand is summarized separately because it is not a guaranteed conversion rate. The arithmetic can show an overpay while the demand signal warns that much of the value sits in items the community currently trades less often.'], points: ['Positive gap: their entered total is higher.', 'Negative gap: your entered total is higher.', 'Fair: the numeric difference is within the visible tolerance, not an official ruling.'] },
    workedExample: { title: 'Worked Example: crop plus pet against one event item', body: ['Imagine your crop and pet rows total 240,000 under the editable references. The other player offers an event item at 250,000. The gap is +10,000 and the percentage is 10,000 ÷ 240,000 = 4.17%, inside the ±7% numerical fair band.', 'If the event item has weak demand while both of your items are easy to move, the 4.17% arithmetic advantage may not compensate for retrading difficulty. Conversely, scarcity or personal collection goals can justify accepting a lower numerical total.'] },
    ...
    limitations: { title: 'Grow a Garden Market Limitations', body: ['The values are community planning references and may not match a specific server, trader, or event cycle. Mutations, crop weight, pets, limited supply, and new updates can move perceived value. The calculator cannot validate item ownership, identify scams, or decide how much personal preference is worth.'], points: ['Editable references require periodic dataset review.', 'A demand score cannot guarantee a quick retrade.', 'The tool does not calculate crop sell formula from a trade row; use the crop value calculator when weight matters.'] },
  ...
  'steal-a-brainrot/value-calculator': {
    ...
    intro: ['The Steal a Brainrot Value Calculator totals a collection by Brainrot, quantity, and mutation while also tracking income rate. It separates asset-style collection value from cash generation so a rare unit with a high reference does not automatically look like the best income producer.', 'The built-in rows are local planning references. Event units, mutations, and the community market can change quickly, so the page labels the dataset as a reference rather than presenting placeholder or sample numbers as official developer values.'],
    ...
    steps: ['Select the Brainrot represented by a local reference row.', 'Set the number of matching units and the mutation state that actually applies.', 'Add different units or mutation states on separate rows.', 'Compare total collection reference with total income instead of choosing from one number alone.'],
    howItWorks: { title: 'How This Steal a Brainrot Value Calculator Works', body: ['For every row, the tool applies the selected mutation multiplier to the Brainrot’s editable value and income references, multiplies by quantity, and adds the contributions. The value total summarizes the entered collection under the current local index; the income total summarizes the resulting per-second production.', 'Because value and income answer different questions, the tool does not collapse them into one opaque score. Value can matter for rarity or trading, while income determines how quickly the collection produces cash toward upgrades or rebirth.'], points: ['Quantity repeats only the same unit and mutation state.', 'Mutation affects the row according to the selected multiplier.', 'Value and income totals remain separate outputs.'] },
    workedExample: { title: 'Worked Example: two matching mutated units', body: ['Suppose a local planning row has value 50,000 and income 2,000 per second. With quantity 2 and a verified 3× mutation, that row contributes 50,000 × 2 × 3 = 300,000 reference value and 2,000 × 2 × 3 = 12,000 income per second. A third unmutated copy should be placed on a separate row so it is not accidentally tripled.'], points: ['At the modeled 12,000 per second, one minute of uninterrupted production is 720,000.', 'The example demonstrates arithmetic, not a current named-unit quote.', 'Use separate rows whenever multiplier states differ.'] },
    ...
    limitations: { title: 'Brainrot Dataset and Mutation Limits', body: ['Local values and demand are community references, while income and mutation behavior may change in updates. The calculator assumes the chosen multiplier applies directly and continuously; collection slots, offline behavior, temporary events, server boosts, and special unit rules can produce different real earnings.'], points: ['The current catalog is intentionally small and needs broader unit coverage.', 'A community value is not an official sale guarantee.', 'The income estimate excludes downtime and collection caps unless the game already includes them in the entered rate.'] },
    referenceData: { title: 'Keep collection metrics separate', note: 'Choose the output that matches the decision being made.', headers: ['Metric', 'Best used for', 'Does not prove'], rows: [['Reference value', 'Collection and trade comparison', 'Guaranteed buyer offer'], ['Income per second', 'Production and payback planning', 'Continuous unattended earnings'], ['Demand', 'Community liquidity context', 'Official rarity or price']] },
    faqs: [
      { question: 'What is the difference between Brainrot value and income?', answer: 'Value is a collection or trade reference. Income is the rate at which the unit produces currency. A unit can rank highly in one metric and less strongly in the other.' },
      ...
      { question: 'Are the Brainrot values official developer prices?', answer: 'No. The page uses a labeled local planning dataset and community context. Event supply and player interest can move trading references.' },
  ...
  'mm2/value-list': {
    ...
    intro: ['The MM2 Value List is the broad searchable reference for the site’s Murder Mystery 2 weapon dataset. It is intended for finding an entry, checking its category and rarity, sorting by the editable value index, and seeing demand beside value before building a trade.', 'MM2 has a player-driven economy rather than an official weapon price table. Every number here is labeled as a community-market planning index. The catalog is deliberately transparent about its limited coverage and should be expanded before it is treated as a comprehensive inventory of MM2 items.'],
    ...
    limitations: { title: 'MM2 Dataset Coverage and Freshness', body: ['The present dataset is a small local reference, not a complete authoritative list of every MM2 weapon and variant. Community lists can disagree, and values may move with demand, duplication concerns, availability, and trading trends. No placeholder number should be read as an official game price.'], points: ['Dataset coverage needs expansion before “complete list” claims are appropriate.', 'Demand labels require periodic editorial review.', 'The page does not verify trades, ownership, or item authenticity.'] },
    ...
    faqs: [
      { question: 'Is this MM2 value list complete?', answer: 'Not yet. It is the site’s broad searchable MM2 reference, but the current local dataset is thin and has been flagged for expansion rather than presented as exhaustive.' },
  ...
  'mm2/trade-calculator': {
    ...
    limitations: { title: 'MM2 Market and Dataset Limits', body: ['Community values change with supply, demand, events, duplication concerns, and list methodology. The site’s current MM2 catalog is limited and needs expansion. A missing weapon should not be replaced with an invented value, and the calculator cannot identify duped items, fraudulent offers, or changes made outside the trade window.'], points: ['The ±7% band is a planning convention, not an MM2 rule.', 'Demand averages do not predict an exact time to trade.', 'Only the rows entered are included in the result.'] },
    ...
    faqs: [
      ...
      { question: 'What if a weapon is missing from the calculator?', answer: 'Do not substitute an unrelated row. The MM2 dataset is flagged for expansion; use a current community reference and treat the URL as unsupported for that item until it is added.' },
    ],
  },
