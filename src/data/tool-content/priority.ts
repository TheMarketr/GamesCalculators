...
export const priorityToolContent = defineToolContent({
  'minecraft/circle-generator': {
    ...
    workedExample: {
      title: 'Worked Example: why a 21-block outline uses 56 blocks',
      body: [
        'Set Diameter to 21, choose Outline, and leave Thickness at 1. The center is block coordinate 10 on the zero-based grid, the outer radius is 10.5, and the inner radius is 9.5. A block is kept only when its center distance is at most 10.5 and greater than 9.5.',
        'The top and bottom rows contain seven blocks each, the next mirrored pair contains four each, and the remaining 17 rows contain two edge blocks each: (7 × 2) + (4 × 2) + (17 × 2) = 56 outline blocks. The result has the same eight-way symmetry builders expect from a midpoint-style pixel circle, while this implementation gets each cell from the stated distance-annulus test.',
      ],
      points: ['The displayed 56-block count is the exact number of highlighted cells.', 'A 2-block outline uses the same outside edge but keeps an additional inner band.', 'Switching to Filled changes the material total because the interior cells are retained.'],
    },
    howToUseResult: {
  ...
  'blox-fruits/value-calculator': {
    ...
    useCases: ['Total several stored fruits before opening a trade discussion.', 'See how quantity changes the Beli weight of one fruit in a bundle.', 'Carry the bundle’s Dealer-price baseline into the two-sided trade calculator.'],
    ...
    howToUseResult: { title: 'How to Use a Fruit Bundle Total', body: ['Use the result as a reproducible Dealer-price baseline for offer construction. Identify the largest Beli contributor and rebuild both sides in the trade calculator instead of comparing a remembered headline number. Community exchange behavior is deliberately outside this one-sided shop-price calculation.'], points: ['Use the Beli total only as the game’s Dealer-price baseline.', 'Recheck quantities before sending a screenshot or trade proposal.', 'Keep the permanent Robux total separate from physical-fruit arithmetic.'] },
    ...
    referenceData: { title: 'Dragon, Kitsune and Buddha bundle arithmetic', note: 'Dealer prices reviewed against the cited Blox Fruits wiki table on September 4, 2026.', headers: ['Fruit', 'Dealer Beli price', 'Permanent Robux price'], rows: [['Dragon', '15,000,000', '5,000'], ['Kitsune', '8,000,000', '4,000'], ['Buddha', '1,200,000', '1,650'], ['Bundle total', '24,200,000', '10,650']] },
    faqs: [
      { question: 'Does the Blox Fruits bundle total include community demand?', answer: 'No. It totals developer-set Dealer Beli prices and permanent Robux prices in separate units. It does not output a community demand score.' },
  ...
  'mm2/value-list': {
    ...
    intro: ['The MM2 Value List searches 300 source-dated Murder Mystery 2 items, including knives, guns, sets, pets and miscellaneous records. It is intended for finding an exact entry, checking its class and rarity, and seeing the cited Supreme Values category beside its current community estimate.', 'MM2 has a player-driven economy rather than an official weapon price table. Every imported record carries its own source URL, review date and status note from the September 2026 snapshot.'],
    ...
    howItWorks: { title: 'How This MM2 Value List Works', body: ['The page filters a local array of source-dated item records. Each record has an item name, category, rarity tier, Supreme Values estimate, demand rating, source link, review date and contextual note. Search matches the item metadata, while sorting reorders the same records; it does not calculate a new market price.', 'Unlike the knife-values and godly-values URLs, this page’s intent is breadth across the current catalog. The focused pages narrow the dataset and add category-specific interpretation rather than serving as alternate copies of this list.'], points: ['Value is the cited community estimate for the row.', 'Demand is a separate liquidity signal.', 'Category and rarity describe the item but do not determine value by themselves.'] },
    ...
    limitations: { title: 'MM2 Dataset Coverage and Freshness', body: ['The 300-row dataset is a dated Supreme Values community snapshot, not a complete authoritative list of every MM2 item and variant. Community lists can disagree, and values may move with demand, duplication concerns, availability, and trading trends.'], points: ['Every row links its source category and review date.', 'Demand labels require periodic review.', 'The page does not verify trades, ownership, or item authenticity.'] },
    referenceData: { title: 'Fields in the MM2 reference', note: 'Each field answers a different lookup question.', headers: ['Field', 'Purpose', 'Do not assume'], rows: [['Category', 'Knife, gun, set, pet or miscellaneous grouping', 'Category alone sets price'], ['Rarity', 'In-game or catalog tier context', 'Every item in a tier is equal'], ['Value', 'Supreme Values community estimate', 'Official MM2 price'], ['Demand', 'Relative community trading interest', 'Guaranteed instant trade']] },
    faqs: [
      { question: 'Is this MM2 value list complete?', answer: 'It contains 300 source-dated community-reference records across multiple item classes. That is a substantial working catalog, but it does not claim all items and variants in the full MM2 economy.' },
      ...
      { question: 'Are the listed MM2 values official?', answer: 'No. They are dated Supreme Values community estimates with a source link on each row. MM2 does not publish an official player-to-player price table.' },
    ],
  },
  ...
  'mm2/trade-calculator': {
    ...
    workedExample: { title: 'Worked Example: Harvester for three adds', body: ['In the September 3 snapshot, Harvester is 250. An offer of Icepiercer at 160, Heartblade at 65 and Luger at 17 totals 242. Their side is 8 lower, or −3.2% of the 250-point Harvester side, so the arithmetic falls within the tool’s ±7% Fair band.', 'The demand field still matters separately: three adds can take more work to retrade than one Harvester. The calculator exposes the eight-point gap but does not hide liquidity inside the total.'] },
    ...
    limitations: { title: 'MM2 Market and Dataset Limits', body: ['Community values change with supply, demand, events, duplication concerns, and list methodology. The calculator uses 300 source-dated Supreme Values records; a missing item must not be replaced with an unrelated entry, and the calculator cannot identify dupes, fraud, or changes outside the trade window.'], points: ['The ±7% band is a planning convention, not an MM2 rule.', 'Demand ratings do not predict an exact time to trade.', 'Only the rows entered are included in the result.'] },
